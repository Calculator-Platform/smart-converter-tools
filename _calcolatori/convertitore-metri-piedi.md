...
description: "Converti metri in miglia e miglia in km istantaneamente. Calcolatore gratuito con tabella di conversione rapida."
category: "convertitori-unita"
subcategory: "lunghezza"
---

<!-- Breadcrumb discreto su sfondo bianco -->
<div class="breadcrumb">Convertitore Metri Piedi m ↔ ft Online Gratis</div>

<div class="page-hero">
  <div class="hero-icon">📐</div>
  <h1>Convertitore Metri Piedi</h1>
  <p>Converti velocemente tra m e piedi</p>
</div>
...
<div class="calculator-box">
  ...
  <div id="m-to-ft" class="converter-panel active">
    <div class="input-wrapper"><label for="m-input">metri (km)</label><input type="number" id="m-input" placeholder="Inserisci valore" step="0.01" min="0"></div>
...
function convertMToFt(){const km=parseFloat(document.getElementById('m-input').value);const r=document.getElementById('m-result');if(isNaN(km)||km<0){r.innerHTML='<div class="error-message">⚠️ Inserisci un numero valido</div>';return}const mi=km*3.28084;r.innerHTML=`<div class="result-success"><div class="result-main">${km.toFixed(2)} km = <strong>${mi.toFixed(2)} miglia</strong></div></div>`}
function convertFtToM(){const mi=parseFloat(document.getElementById('ft-input').value);const r=document.getElementById('ft-result');if(isNaN(mi)||mi<0){r.innerHTML='<div class="error-message">⚠️ Inserisci un numero valido</div>';return}const km=mi*0.3048;r.innerHTML=`<div class="result-success"><div class="result-main">${mi.toFixed(2)} mi = <strong>${km.toFixed(2)} metri</strong></div></div>`}
...
<h3>Quando serve convertire metri in miglia</h3>
<p>La conversione tra metri e miglia è essenziale per misurazioni di altezza, distanze brevi e specifiche tecniche. Se stai pianificando un viaggio negli Stati Uniti, dove si usano i piedi.

<h3>Formula di conversione</h3>
<p><strong>Da metri a miglia:</strong> 1 km = 3.28084 miglia<br><strong>Da miglia a metri:</strong> 1 miglio = 0.3048 km</p>
