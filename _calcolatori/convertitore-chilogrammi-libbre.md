...
<div class="calculator-box">
  <div class="converter-tabs">
    <button class="tab-btn active" onclick="switchTab('a')"><span class="tab-label">Chilogrammi → Libbre</span></button>
    <button class="tab-btn" onclick="switchTab('b')"><span class="tab-label">Libbre → Chilogrammi</span></button>
  </div>
  
  <div id="a" class="converter-panel active">
    <div class="input-wrapper"><label for="in1">Chilogrammi (kg)</label><input type="number" id="in1" placeholder="Inserisci valore" step="0.01" min="0"></div>
    <button class="calc-button" onclick="conv1()"><span class="button-icon">⚡</span>Converti in Libbre</button>
    <div id="res1" class="result-box"></div>
  </div>
  
  <div id="b" class="converter-panel">
    <div class="input-wrapper"><label for="in2">Libbre (lb)</label><input type="number" id="in2" placeholder="Inserisci valore" step="0.01" min="0"></div>
    <button class="calc-button" onclick="conv2()"><span class="button-icon">⚡</span>Converti in Chilogrammi</button>
...
function conv2(){const v=parseFloat(document.getElementById('in2').value);const r=document.getElementById('res2');if(isNaN(v)||v<0){r.innerHTML='<div class="error-message">⚠️ Inserisci un numero valido</div>';return}const res=v*0.453592;r.innerHTML=`<div class="result-success"><div class="result-main">${v.toFixed(2)} ft = <strong>${res.toFixed(2)} metri</strong></div></div>`}
